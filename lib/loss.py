...
def L2_NORM(b): return torch.sqrt(torch.sum((b + 1e-8) ** 2))
...
class MEF_SSIM_Loss(nn.Module):
    ...
    def forward(self, y_1, y_2, y_f):
        ...
        s_bar = (self.w_fn(y_1) * s_1 + self.w_fn(y_2) * s_2) / \
            (self.w_fn(y_1) + self.w_fn(y_2))
        ...
        score = self._ssim(y_hat, y_f, window,
